...
const escapeMD = require('./escape-markdown.js').escapeMarkdown; // our module to escape markdown control characters

exports.pagewriter = function(cat, jsonFeed) {
  const theFeed = jsonFeed;
  const category = cat.replace(/&/g, '&amp;');
  // TODO use categoryToUrl from app.js after #66
  const filename = cat.toLowerCase().replace(/ |'|&/g, '_');
