...
exports.createSidebar = function (categories, directory) {
  ...
  sidebar += '</nav>';
  fs.writeFile(path.resolve(directory, './_includes/sidebar-secondary.html'), sidebar, function (err) {
    'use strict';
    if (err) {
...
exports.createBlogIndex = function (blogs, directory) {
  let blogIndex = '---\n' +
    'layout: page\n' +
    'title: Blog Index \n' +
    '---\n\n' +
  ...
  blogIndex += '</ul>\n';
  fs.writeFile(path.resolve(directory, './blogindex.html'), blogIndex, {
    mode: 0o664
  }, function (err) {
    if (err) {
